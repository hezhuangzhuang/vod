...
public class ListErrorTransformer<T> implements Observable.Transformer<NewDataList<T>, List<T>> {
    @Override
    public Observable<List<T>> call(Observable<NewDataList<T>> httpResponseObservable) {
        return httpResponseObservable.map(new Func1<NewDataList<T>, List<T>>() {
            @Override
            public List<T> call(NewDataList<T> response) {
